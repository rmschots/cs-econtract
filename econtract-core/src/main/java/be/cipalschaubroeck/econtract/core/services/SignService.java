package be.cipalschaubroeck.econtract.core.services;
...
import be.cipalschaubroeck.econtract.core.mappers.DigitalSignatureServiceSessionMapper;
import be.cipalschaubroeck.econtract.core.repository.SignSessionDataRepository;
...
//        DigitalSignatureServiceSession session = uploadDocument(pdfData, "application/pdf");
    ...
    private byte[] getXmlData() {
        try {
            return IOUtils.toByteArray(getClass().getClassLoader().getResourceAsStream("sign-me.xml"));
        } catch (IOException e) {
            throw new IllegalArgumentException("could not get xml");
        }
    }
}
